...
import { FOOD_DB, FOOD_CATEGORIES, INITIAL_USER_INFO, INITIAL_CUSTOM_SUPPS, INITIAL_PHASES } from '../diet/constants.js'; //
...
export const state = {
    ...
    // [신규 추가] 스마트 매크로 변환기의 마지막 선택 상태(기준 식품 + 중량) 영속화용 스키마
    smartCalc: {
        carb: { src: '백미', amt: 130 },
        pro: { src: '닭가슴살(익힘)', amt: 150 },
        fat: { src: '아몬드', amt: 15 }
    }
};
...
export function applyCustomSuppsToDB() {
    // [고스트 항목 방지] 이름 변경/삭제로 더 이상 존재하지 않는 예전 보충제 이름을 먼저 foodDB에서 지운다 —
    // 그렇지 않으면 개명/삭제된 보충제의 옛 이름이 foodDB에 유령처럼 남아 식단 항목 선택지나 엑셀 내보내기
    // 드롭다운 목록에 계속 나타나는 문제가 있었다.
    (state.foodCategories['보충제'] || []).forEach(oldName => { delete state.foodDB[oldName]; });
