...
import { triggerSave, saveToLocal, saveFileNative } from '../core/services.js';
...
/**
 * [버그 수정] 탄/단/지 비율을 "10분율" 정수 3개로 변환. 이전에는 탄/단 비율을 각각 반올림한 뒤 지방을
 * "나머지(10 - 탄 - 지)"로 계산해서, 지방 비중이 아주 낮은 날엔 "3:8:-1"처럼 음수가 나올 수 있었다.
 * 최대 잉여분 배분법(largest remainder method)으로 세 값 모두 0 이상이면서 합이 정확히 10이 되도록 한다.
 */
function computeMacroRatio10(cK, pK, fK, sum) {
    if (sum <= 0) return '0:0:0';
    const raw = [cK, pK, fK].map(v => (v / sum) * 10);
    const floors = raw.map(Math.floor);
    let remainder = 10 - floors.reduce((a, b) => a + b, 0);
            ...
            <div ${toggleAttr} class="p-3.5 flex justify-between items-center ${startExpanded ? '' : 'cursor-pointer hover:bg-slate-900/40'} transition-colors select-none">
                <div class="flex items-center gap-2.5 min-w-0">
                    ...
                    <div class="truncate">
                        ...
                        ${data.bf > 0 ? `<span class="text-xs font-bold text-amber-400 ml-1.5">체지방 ${data.bf.toFixed(1)}%</span>` : ''}
                    </div>
                </div>
...
export function openRecordModal(editDateStr = '') {
    ...
    if (editDateStr) {
        ...
        document.getElementById('record-weight-input').value = data.weight || ''; document.getElementById('record-bf-input').value = data.bf || '';
        ...
        syncQuickNoteChipVisuals(data.specialNote ? data.specialNote.split(',').map(s => s.trim()).filter(Boolean) : []);
        ...
        document.getElementById('record-weight-input').value = ''; document.getElementById('record-bf-input').value = '';
...
export function toggleQuickNoteChip(tag) {
    ...
    syncQuickNoteChipVisuals(tokens);
}
/** [버그수정] 태그 칩을 눌러도 선택 여부가 시각적으로 표시되지 않던 문제 — 텍스트 입력값만 바뀌고
 * 칩 자체는 항상 기본 색으로 남아있었다. 현재 선택된 태그 목록을 기준으로 칩 배경색을 다시 맞춘다. */
function syncQuickNoteChipVisuals(tokens) {
    document.querySelectorAll('.chip-note-tag').forEach(btn => {
        btn.classList.toggle('matrix-chip-active', tokens.includes(btn.textContent.trim()));
    });
}
...
export function pullDietaryMacrosFromPlanner() {
    ...
    setTimeout(() => {
        ...
        document.getElementById('record-ratio-display').innerText = computeMacroRatio10(cK, pK, fK, sum);
...
export function saveWeightRecordData() {
    ...
    target.weight = weightVal; target.bf = parseFloat(document.getElementById('record-bf-input').value) || 0; target.dayOfWeek = document.getElementById('record-day-display').value;
    ...
    const profWeightEl = document.getElementById('prof-weight-display'); // [빠른 설정 FAB] 운동 캘린더(calendar.html) 화면에는 이 요소 자체가 없다
    if (profWeightEl && state.workouts[activeToday] && state.workouts[activeToday].weight > 0) { profWeightEl.innerText = state.workouts[activeToday].weight.toFixed(2) + 'kg'; }
    showToast("기록 저장 완료.");
}
...
export function deleteWeightRecordData(dateStr) {
    if (confirm(`[${dateStr}] 기록을 삭제할까요?\n(등록된 운동 일지는 그대로 보존됩니다.)`)) {
        ...
        if (t) { t.weight = 0; t.bf = 0; t.weightDelta = 0; t.visualScore = 0; t.restingHR = 0; t.sleepTime = 0; t.workoutPart = ''; t.anaerobic = 0; t.aerobic = 0; t.water = 0; t.bowel = 'X'; t.carbs = 0; t.protein = 0; t.fat = 0; t.totalKcal = 0; t.macroRatio = '0:0:0'; t.specialNote = ''; t.memo = ''; }
...
export function updateWeightTrendChart() {
    ...
    // [점 크기 최적화] 1개월/6개월/전체처럼 데이터가 많아지면 점들이 서로 겹쳐 뭉개져 보기 불편했다 —
    // 표시되는 날짜 수에 따라 점 반경을 단계적으로 줄이고, 30개를 넘으면 점을 아예 숨기고 선만 남긴다.
    const pointCount = rangeDates.length;
    const pr = pointCount > 30 ? 0 : pointCount > 14 ? 2 : 4;
    const prHover = pointCount > 30 ? 3 : pointCount > 14 ? 4 : 6;
    ...
    if (filterMode === 'all') {
        datasets = [
            { type: 'line', label: '공복체중(kg)', data: rangeDates.map(d => state.workouts[d].weight), borderColor: '#0EA5E9', backgroundColor: 'transparent', borderWidth: 3, pointBackgroundColor: '#0EA5E9', pointRadius: pr, pointHoverRadius: prHover, yAxisID: 'yLeft', tension: 0.25 },
        ...
        datasets = [
            { type: 'line', label: '공복체중(kg)', data: rangeDates.map(d => state.workouts[d].weight), borderColor: '#0EA5E9', backgroundColor: 'transparent', borderWidth: 3, pointBackgroundColor: '#0EA5E9', pointRadius: pr, pointHoverRadius: prHover, yAxisID: 'yLeft', tension: 0.1 },
            // [체지방 기록 추가] 체중 추이와 함께 체지방률도 같은 그래프에서 비교할 수 있도록 별도 축(yBf)에 표시.
            // 기록이 없는 날짜는 null로 둬 spanGaps로 이어 그린다(0%로 왜곡되지 않도록).
            { type: 'line', label: '체지방률(%)', data: rangeDates.map(d => state.workouts[d].bf > 0 ? state.workouts[d].bf : null), borderColor: '#F59E0B', backgroundColor: 'transparent', borderWidth: 2, borderDash: [5, 3], pointBackgroundColor: '#F59E0B', pointRadius: pr, pointHoverRadius: prHover, yAxisID: 'yBf', tension: 0.2, spanGaps: true },
        ...
        // [가독성 수정] 오른쪽에 축이 2개(체지방%/체중변화kg) 나란히 붙어 어떤 눈금이 어떤 데이터인지
        // 구분이 안 됐다 — 각 축에 짧은 단위 타이틀을 붙여 시각적으로 구분되게 한다.
        optionsScales.yBf = { position: 'right', grid: { display: false }, ticks: { color: '#F59E0B', font: { size: 9 } }, title: { display: true, text: '%', color: '#F59E0B', font: { size: 9, weight: 'bold' } } };
        optionsScales.yDelta = { position: 'right', grid: { display: false }, ticks: { color: '#94A3B8', font: { size: 10 } }, title: { display: true, text: 'Δkg', color: '#94A3B8', font: { size: 9, weight: 'bold' } } };
        ...
        datasets = [
            ...
            { type: 'line', label: '탄수화물(g)', data: rangeDates.map(d => state.workouts[d].carbs || 0), borderColor: '#F59E0B', borderWidth: 2, pointRadius: pr, pointHoverRadius: prHover, backgroundColor: 'transparent', yAxisID: 'yRight', tension: 0.2 },
            { type: 'line', label: '단백질(g)', data: rangeDates.map(d => state.workouts[d].protein || 0), borderColor: '#10B981', borderWidth: 2, pointRadius: pr, pointHoverRadius: prHover, backgroundColor: 'transparent', yAxisID: 'yRight', tension: 0.2 },
            { type: 'line', label: '지방(g)', data: rangeDates.map(d => state.workouts[d].fat || 0), borderColor: '#0EA5E9', borderWidth: 2, pointRadius: pr, pointHoverRadius: prHover, backgroundColor: 'transparent', yAxisID: 'yRight', tension: 0.2 }
        ];
        ...
        datasets = [
            { type: 'line', label: '종합컨디션(점)', data: rangeDates.map(d => state.workouts[d].condition || 7), borderColor: '#0EA5E9', borderWidth: 2.5, pointRadius: pr, pointHoverRadius: prHover, backgroundColor: 'transparent', yAxisID: 'yLeft', tension: 0.3 },
            { type: 'line', label: '눈바디점수(점)', data: rangeDates.map(d => state.workouts[d].visualScore || 5), borderColor: '#A855F7', borderWidth: 2.5, pointRadius: pr, pointHoverRadius: prHover, backgroundColor: 'transparent', yAxisID: 'yLeft', tension: 0.3 },
            { type: 'line', label: '수면시간(h)', data: rangeDates.map(d => state.workouts[d].sleepTime || 0), borderColor: '#64748B', borderWidth: 1.5, borderDash: [4, 4], pointRadius: pr, pointHoverRadius: prHover, backgroundColor: 'transparent', yAxisID: 'yRight', tension: 0.1 }
        ];
...
function updateKpiSnapshotCards() {
    ...
    // [방어 코드 보강] 이전에는 wLbl만 null 체크하고 나머지 요소는 그대로 참조해서, 이 카드들 중 일부만
    // 존재하는 화면에서 호출되면 크래시가 날 수 있었다 — 모든 대상 요소를 미리 조회해 개별적으로 가드한다.
    const wLbl = document.getElementById('kpi-display-weight');
    const kcalLbl = document.getElementById('kpi-display-kcal');
    const subWeight = document.getElementById('kpi-sub-weight');
    const subMacros = document.getElementById('kpi-sub-macros');
    const condLbl = document.getElementById('kpi-display-cond');
    const bowelLbl = document.getElementById('kpi-display-bowel');
    if (!wLbl || !kcalLbl) return;
    if (sorted.length === 0) { wLbl.innerText = "-- kg"; kcalLbl.innerText = "-- kcal"; return; }
    const recent7 = sorted.slice(0, 7); let sumW = 0, sumK = 0, sumC = 0, sumP = 0, sumF = 0, sumSleep = 0, sumCond = 0, bowelO = 0, sumBf = 0, bfCount = 0;
    recent7.forEach(d => { const o = state.workouts[d]; sumW += o.weight; sumK += o.totalKcal || 0; sumC += o.carbs || 0; sumP += o.protein || 0; sumF += o.fat || 0; sumSleep += o.sleepTime || 0; sumCond += o.condition || 7; if (o.bowel === 'O') bowelO++; if (o.bf > 0) { sumBf += o.bf; bfCount++; } }); const len = recent7.length;
    wLbl.innerText = `${(sumW/len).toFixed(1)} kg`;
    // [체지방 기록 추가] 체지방률이 기록된 날짜만 평균 내어 최근 변화량 옆에 함께 보여준다.
    const bfSubText = bfCount > 0 ? ` · 평균 체지방률: ${(sumBf/bfCount).toFixed(1)}%` : '';
    if (subWeight) subWeight.innerText = `최근 기록 변화량: ${(state.workouts[sorted[0]].weightDelta || 0).toFixed(1)} kg${bfSubText}`;
    kcalLbl.innerText = `${Math.round(sumK/len).toLocaleString()} kcal`;
    if (subMacros) subMacros.innerText = `주간평균 탄:${Math.round(sumC/len)}g 단:${Math.round(sumP/len)}g 지:${Math.round(sumF/len)}g`;
    if (condLbl) condLbl.innerText = `평균 수면: ${(sumSleep/len).toFixed(1)}h | 컨디션: ${(sumCond/len).toFixed(1)}점`;
    if (bowelLbl) bowelLbl.innerText = `배변 빈도: ${Math.round((bowelO/len)*100)}%`;
}
...
export async function exportWeightRecordsToCSV() {
    ...
    setTimeout(async () => {
        let csvContent = "﻿";
        const headers = ["일자", "요일", "공복체중(kg)", "체지방률(%)", "체중변화량(kg)", "수면시간(시간)", "컨디션(1-10)", "눈바디점수(1-10)", "공복심박수(bpm)", "운동부위", "탄수화물(g)", "단백질(g)", "지방(g)", "총섭취칼로리(kcal)", "탄단지비율", "수분섭취(L)", "근력운동(분)", "유산소(분)", "배변활동(O/X)", "특이사항", "메모"];
        ...
        dates.forEach(dateStr => {
            ...
            const row = [dateStr, d.dayOfWeek || "", d.weight.toFixed(2), (d.bf || 0).toFixed(1), (d.weightDelta || 0).toFixed(2), d.sleepTime || 0, d.condition || 7, d.visualScore || 5, d.restingHR || 60, d.workoutPart ? `"${d.workoutPart.replace(/"/g, '""')}"` : '""', d.carbs || 0, d.protein || 0, d.fat || 0, d.totalKcal || 0, d.macroRatio || "0:0:0", d.water || 0, d.anaerobic || 0, d.aerobic || 0, d.bowel || "X", sNote, memoStr];
            csvContent += row.join(",") + "\n";
        });
        ...
        try {
            await saveFileNative(fileName, csvContent, 'text/csv', showToast);
    ...
    reader.onload = function (e) {
        try {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                t.weight = weight; t.dayOfWeek = row[1].replace(/"/g, '') || ""; t.bf = parseFloat(row[3]) || 0; t.sleepTime = parseFloat(row[5]) || 0;
                t.condition = parseInt(row[6]) || 7; t.visualScore = parseInt(row[7]) || 5; t.restingHR = parseInt(row[8]) || 60;
                t.workoutPart = row[9] ? row[9].replace(/"/g, '') : ""; t.carbs = parseFloat(row[10]) || 0; t.protein = parseFloat(row[11]) || 0;
                t.fat = parseFloat(row[12]) || 0; t.totalKcal = parseInt(row[13]) || 0; t.macroRatio = row[14] ? row[14].replace(/"/g, '') : "0:0:0";
                t.water = parseFloat(row[15]) || 0; t.anaerobic = parseInt(row[16]) || 0; t.aerobic = parseInt(row[17]) || 0; t.bowel = row[18] ? row[18].replace(/"/g, '') : "X";
                t.specialNote = row[19] ? row[19].replace(/"/g, '') : ""; t.memo = row[20] ? row[20].replace(/"/g, '') : ""; count++;
            }
