...
let autoCloseTimeout = null; // [알람창 자동 닫힘] 유한 반복이 다 끝난 뒤 3초 후 stopRestTimer()를 예약해둔 타이머
...
// [버벅임 방지] 재생마다 새 AudioContext를 만들면 매번 초기화 비용이 들어 소리가 밀리거나 끊길 수 있다.
// 하나를 지연 생성해 재사용하고, 브라우저가 자동으로 suspend 시켰다면 재생 직전 resume() 한다.
...
function triggerAlarmRing(soundType) {
    ...
    const ring = () => {
        ...
        triggerVibration();
        if (alarmRingCount >= maxRings) {
            if (alarmAudioInterval) clearInterval(alarmAudioInterval);
            // [알람창 자동 닫힘] "계속"이 아닌 유한 반복을 다 채웠을 때만, 3초 후 부드럽게(기존
            // transition-all duration-500 클래스 그대로) 알람창을 닫는다. 그 전에 사용자가 대기/정지를
            // 누르면 stopRestTimer/extendRestTimer가 이 예약을 먼저 지운다.
            if (maxRings !== Infinity) {
                if (autoCloseTimeout) clearTimeout(autoCloseTimeout);
                autoCloseTimeout = setTimeout(() => { autoCloseTimeout = null; stopRestTimer(); }, 3000);
            }
        }
    };
...
export function previewAlarmSound(soundType) {
    playAudioTone(soundType || '1');
}
...
export function stopRestTimer() {
    ...
    if (alarmAudioInterval) clearInterval(alarmAudioInterval);
    document.getElementById('timer-floating-bar').className = "fixed bottom-0 left-60 right-0 z-[70] transform translate-y-full opacity-0 transition-all duration-500 pointer-events-none";
}
...
export function startTimerLogic(seconds, soundType) {
    ...
    bar.className = "fixed bottom-0 left-60 right-0 z-[70] transform translate-y-0 opacity-100 transition-all duration-500 pointer-events-auto shadow-[0_-10px_40px_rgba(245,158,11,0.2)]";
